    ...
    for file_name in cal_images:
        ...
        if ret==True:
            ...
            if save_images_flag:
                ...
                result_filename = os.path.join("../output_images/chessboard_images", root + "-corners" + ext)
    
                #save the result image
                mpimg.imsave(result_filename, img)
    
    #concatenate arrays for object points and image points            
    #objpoints = np.concatenate(objpoints)
    #imgpoints = np.concatenate(imgpoints)
    
    #calibrate camera: calculate camera matrix mtx and distortion coefficients dist  
